...
import argon2
from fastapi import APIRouter, Cookie, HTTPException, Depends, Body
...
@router.post("/login", response_model=db.user.UserDTO)
async def login(response: Response, input: input_models.LoginInput = Body(...)):
    user = db.user.find_by_username(input.username)
    if user is None:
        raise HTTPException(
            status_code=401,
...
async def logout(
    res: Response,
    session_id: Annotated[Optional[str], Cookie(alias=config.COOKIE_NAME)] = None,
):
